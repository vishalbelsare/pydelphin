...
from delphin.vpm import (load, VPM)


def test_type_map_single_rule_no_semi():
    assert load(S('event <> e')).apply('event2', {}) == ('e2', {})
    assert load(S('event >> e')).apply('event2', {}) == ('e2', {})
    assert load(S('event << e')).apply('event2', {}) == ('event2', {})
    assert load(S('event == e')).apply('event2', {}) == ('e2', {})
    assert load(S('event => e')).apply('event2', {}) == ('e2', {})
    assert load(S('event <= e')).apply('event2', {}) == ('event2', {})

    assert load(S('event <> e')).apply('event2', {}, reverse=True) == ('event2', {})
    assert load(S('event >> e')).apply('event2', {}, reverse=True) == ('event2', {})
    assert load(S('event << e')).apply('event2', {}, reverse=True) == ('event2', {})
    assert load(S('event == e')).apply('event2', {}, reverse=True) == ('event2', {})
    assert load(S('event => e')).apply('event2', {}, reverse=True) == ('event2', {})
    ...
    assert load(S('event <> e')).apply('e2', {}) == ('e2', {})
    assert load(S('event >> e')).apply('e2', {}) == ('e2', {})
    assert load(S('event << e')).apply('e2', {}) == ('e2', {})
    assert load(S('event == e')).apply('e2', {}) == ('e2', {})
    assert load(S('event => e')).apply('e2', {}) == ('e2', {})
    ...
    # assert load(S('event <> e')).apply('e2', {}, reverse=True) == ('event2', {})
    # assert load(S('event >> e')).apply('e2', {}, reverse=True) == ('e2', {})
    # assert load(S('event << e')).apply('e2', {}, reverse=True) == ('event2', {})
    # assert load(S('event == e')).apply('e2', {}, reverse=True) == ('event2', {})
    # assert load(S('event => e')).apply('e2', {}, reverse=True) == ('e2', {})
    # assert load(S('event <= e')).apply('e2', {}, reverse=True) == ('event2', {})
    ...
    assert load(S('* >> a')).apply('event2', {}) == ('a2', {})
    assert load(S('semarg << *')).apply('event2', {}) == ('event2', {})
    # assert load(S('semarg << *')).apply('a2', {}, reverse=True) == ('semarg2', {})
    assert load(S('semarg << *')).apply('a2', {}, reverse=True) == ('a2', {})


def test_type_map_multi_rule_no_semi():
    assert load(S('event <> e\nref-ind <> x')).apply('event2', {}) == ('e2', {})
    assert load(S('event <> e\nref-ind <> x')).apply('ref-ind4', {}) == ('x4', {})

    assert load(S('event >> e\nevent >> wrong')).apply('event2', {}) == ('e2', {})
    assert load(S('event >> e\ne >> wrong')).apply('event2', {}) == ('e2', {})
    assert load(S('event << wrong\nevent >> e')).apply('event2', {}) == ('e2', {})
...
def test_prop_map_no_semi():
    ...
    v = load(S('E.ASPECT.SOON E.ASPECT.EVER E.ASPECT.ALREADY : TENSE\n'
                   '  bool bool +    <> past\n'
